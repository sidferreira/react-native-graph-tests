...
import {LinePath as DefaultLinePath} from './LinePath';
import {
  createGraphPath,
...
export function AnimatedLineGraph({
  ...
  const basePanGesture = usePanGesture({holdDuration: 300});
  const {gesture, isActive, x} = propPanGesture || basePanGesture;
  ...
  const pathRange: GraphPathRange = useMemo(
    () => getGraphPathRange(points, range, valueKey),
    [points, range, valueKey],
  );
  ...
  const DetectorComponent = enablePanGesture ? GestureDetector : Fragment;

  return (
    <View {...props}>
      <DetectorComponent gesture={enablePanGesture ? gesture : {}}>

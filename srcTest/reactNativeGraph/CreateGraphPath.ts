...
export function getGraphPathRange(
  ...
  const minValueY =
    range?.y?.min ??
    points.reduce(
      (prev, curr) => (curr[field] < prev ? curr[field] : prev),
...
function createGraphPathBase({
  ...
  const getGraphPoint = (point: GraphPoint): Vector => {
    ...
    const y =
      actualHeight -
      actualHeight * pixelFactorY(point[field], range.y.min, range.y.max) +
      verticalPadding;
  ...
  points.forEach((point, i) => {
    ...
    if (point.date < range.x.min || point.date > range.x.max) {
      return;
    }
